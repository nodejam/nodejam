...
class BaseModel
        ...
        if not @_id?
            if meta.autoInsertedFields
                if meta.autoInsertedFields.created
                    @[meta.autoInsertedFields.created] = Date.now()
                if meta.autoInsertedFields.updated
                    @[meta.autoInsertedFields.updated] = Date.now()                        
        else
            if meta.autoInsertedFields
                if meta.autoInsertedFields.updated
                    @[meta.autoInsertedFields.updated] = Date.now()                        
    ...
    validate: =>
        ...
        if meta.autoInsertedFields
            if meta.autoInsertedFields.created
                if isNaN @[meta.autoInsertedFields.created]
                    errors.push "Invalid #{meta.autoInsertedFields.created}."
                if isNaN @[meta.autoInsertedFields.updated]
