(function() {
    ...
    Connector.prototype.getEntitySchema = function*() {
        ...
        if (!this.ctor.__entitySchema) {
            this.ctor.__entitySchema = yield* schemaManager.getEntitySchema(this.ctor.entitySchema.schema.id);
        }
    ...
    Connector.prototype.getEntitySchemaFromRecord = function*(record) {
        ...
        if (record) {
            if (record.getEntitySchema) {
                ...
                return yield* schemaManager.getEffectiveEntitySchema(record, typeDef);
            }
        }
    ...
    Connector.prototype.findById = function*(id) {
        ...
        return yield* database.findOne(entitySchema, query, {}, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.find = function*(query, options) {
        var services = getServices();
        var entitySchema = yield* this.getEntitySchema();
        return yield* database.find(entitySchema, query, options, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.findOne = function*(query, options) {
        var services = getServices();
        var entitySchema = yield* this.getEntitySchema();
        return yield* database.findOne(entitySchema, query, options, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.count = function*(query) {
        var services = getServices();
        var entitySchema = yield* this.getEntitySchema();
        return yield* database.count(entitySchema, query, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.destroyAll = function*(query) {
        var services = getServices();
        var entitySchema = yield* this.getEntitySchema();
        return yield* database.destroyAll(entitySchema, query, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.save = function*(record) {
        ...
        return yield* database.save(record, entitySchema, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.destroy = function*(record) {
        var services = getServices();
        var entitySchema = yield* this.getEntitySchemaFromRecord(record);
        return yield* database.destroy(record, entitySchema, services.schemaManager, services.db);
    };
    ...
    Connector.prototype.link = function*(record, name) {
        var services = getServices();
        var entitySchema = yield* this.getEntitySchemaFromRecord(record);
        return yield* database.link(record, entitySchema, name, services.schemaManager, services.db);
    };
